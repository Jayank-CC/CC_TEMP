(function () {
  ...
  // ---- "Why Choose Us" screenshot carousel (simple auto-advancing track) ----
  const workTrack = document.querySelector(".ecom-carousel-track");
  const workPrev = document.querySelector(".ecom-carousel-prev");
  const workNext = document.querySelector(".ecom-carousel-next");
  let workIndex = 0;
  ...
  if (workTrack && !window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
    window.setInterval(function () { showWork(workIndex + 1); }, 4000);
  }

  // ---- Testimonial carousel (bounded drag + autoplay + clone-based infinite loop) ----
  const viewport = document.querySelector(".ecom-review-viewport");
  const track = document.querySelector(".ecom-review-track");
  ...
  function measure() {
    const card = track.querySelector(".ecom-review-card");
    if (!card) return;
  ...
  if (window.__ecomPageInit) {
    build();
  } else {
    window.__ecomPageInit = true;
