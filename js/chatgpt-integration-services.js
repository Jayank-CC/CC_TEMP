(function () {
  ...
  function initAccordion() {
    const items = document.querySelectorAll(".cgpt-acc-item");
    if (!items.length) return;

    items.forEach(function (item) {
      const btn = item.querySelector(".cgpt-acc-btn");
      const panel = item.querySelector(".cgpt-acc-panel");
      if (!btn || !panel || btn.dataset.cgptBound === "1") return;
      btn.dataset.cgptBound = "1";
      ...
      btn.addEventListener("click", function () {
        ...
        items.forEach(function (other) {
          ...
          const otherBtn = other.querySelector(".cgpt-acc-btn");
          const otherPanel = other.querySelector(".cgpt-acc-panel");
  ...
  function reviewCarousel() {
    const viewport = document.querySelector(".cgpt-review-viewport");
    const track = document.querySelector(".cgpt-review-track");
    ...
    function measure() {
      const card = track.querySelector(".cgpt-review-card");
      if (!card) return;
  ...
  if (window.__cgptPageInit) {
    init();
  } else {
    window.__cgptPageInit = true;
    if (document.readyState === "loading") {
