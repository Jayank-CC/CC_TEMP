(function(){
  ...
  const workTrack=document.querySelector(".cwd-work-track");
  const workPrev=document.querySelector(".cwd-work-prev");
  const workNext=document.querySelector(".cwd-work-next");
  let workIndex=0;
  function showWork(index){
    if(!workTrack)return;
    const count=workTrack.children.length;
    workIndex=(index+count)%count;
  ...
  const statGrid=document.querySelector(".cwd-stat-grid");
  if(statGrid){statGrid.classList.remove("cwd-reveal","is-visible")}
  const revealTargets=document.querySelectorAll(".cwd-service-card");
  revealTargets.forEach(function(target){target.classList.add("cwd-reveal")});
  if("IntersectionObserver" in window&&!window.matchMedia("(prefers-reduced-motion: reduce)").matches){
    const revealObserver=new IntersectionObserver(function(entries){entries.forEach(function(entry){if(entry.isIntersecting){entry.target.classList.add("is-visible");revealObserver.unobserve(entry.target)}})},{threshold:.12});
  ...
  const viewport=document.querySelector(".cwd-review-viewport");
  const track=document.querySelector(".cwd-review-track");
  ...
  function measure(){const card=track.querySelector(".cwd-review-card");if(!card)return;const styles=getComputedStyle(track);step=card.getBoundingClientRect().width+(parseFloat(styles.columnGap||styles.gap)||0);position(false,0)}
  ...
  function build(){lastBreakpoint=breakpoint();visible=lastBreakpoint==="mobile"?1:2;stop();window.clearTimeout(settleId);moving=false;track.replaceChildren();originals.slice(-visible).forEach(function(card){const clone=card.cloneNode(true);clone.setAttribute("aria-hidden","true");track.appendChild(clone)});originals.forEach(function(card){track.appendChild(card)});originals.slice(0,visible).forEach(function(card){const clone=card.cloneNode(true);clone.setAttribute("aria-hidden","true");track.appendChild(clone)});index=visible+2;requestAnimationFrame(function(){measure();start()})}
  ...
  let resizeId=0;window.addEventListener("resize",function(){window.clearTimeout(resizeId);resizeId=window.setTimeout(function(){if(breakpoint()!==lastBreakpoint)build();else{moving=false;normalize();measure()}},120)});
  build();
}());
