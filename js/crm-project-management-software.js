(function () {
  ...
  // Adapted verbatim from js/ecommerce-development-services.js's own review carousel (same
  // shared review-card content pool), renamed from the `ecom-`/`mkt-` prefix to `crm-` to match
  // this page's own class names.
  const viewport = document.querySelector(".crm-review-viewport");
  const track = document.querySelector(".crm-review-track");
  ...
  function measure() {
    const card = track.querySelector(".crm-review-card");
    if (!card) return;
  ...
  if (window.__crmPageInit) {
    build();
  } else {
    window.__crmPageInit = true;
