(function () {
  ...
  function initAccordion() {
    const items = document.querySelectorAll(".shpi-acc-item");
    if (!items.length) return;

    items.forEach(function (item) {
      const btn = item.querySelector(".shpi-acc-btn");
      const panel = item.querySelector(".shpi-acc-panel");
      if (!btn || !panel || btn.dataset.shpiBound === "1") return;
      btn.dataset.shpiBound = "1";
      ...
      btn.addEventListener("click", function () {
        ...
        items.forEach(function (other) {
          ...
          const otherBtn = other.querySelector(".shpi-acc-btn");
          const otherPanel = other.querySelector(".shpi-acc-panel");
  ...
  // ---- Consultation form (honeypot + fake success) ----
  function initConsultForm() {
    const form = document.querySelector(".shpi-consult-form");
    if (!form || form.dataset.shpiBound === "1") return;
    form.dataset.shpiBound = "1";

    form.addEventListener("submit", function (e) {
      e.preventDefault();
      const honeypot = form.querySelector('input[name="shpi_hp"]');
      if (honeypot && honeypot.value) return;

      const success = form.querySelector(".shpi-form-success");
  ...
  function reviewCarousel() {
    const viewport = document.querySelector(".shpi-review-viewport");
    const track = document.querySelector(".shpi-review-track");
    ...
    function measure() {
      const card = track.querySelector(".shpi-review-card");
      if (!card) return;
  ...
  if (window.__shopifyIntegrationPageInit) {
    init();
  } else {
    window.__shopifyIntegrationPageInit = true;
    if (document.readyState === "loading") {
