(function () {
  ...
  // ---- Case studies tabs (single-open, click to switch) ----
  function initCaseTabs() {
    const tabs = document.querySelectorAll(".azc-cases-tab");
    const panels = document.querySelectorAll(".azc-cases-panel");
    if (!tabs.length || !panels.length) return;

    tabs.forEach(function (tab) {
      if (tab.dataset.azcBound === "1") return;
      tab.dataset.azcBound = "1";
  ...
  function initAccordion() {
    const items = document.querySelectorAll(".azc-acc-item");
    if (!items.length) return;

    items.forEach(function (item) {
      const btn = item.querySelector(".azc-acc-btn");
      const panel = item.querySelector(".azc-acc-panel");
      if (!btn || !panel || btn.dataset.azcBound === "1") return;
      btn.dataset.azcBound = "1";
      ...
      btn.addEventListener("click", function () {
        ...
        items.forEach(function (other) {
          ...
          const otherBtn = other.querySelector(".azc-acc-btn");
          const otherPanel = other.querySelector(".azc-acc-panel");
  ...
  function reviewCarousel() {
    const viewport = document.querySelector(".azc-review-viewport");
    const track = document.querySelector(".azc-review-track");
    ...
    function measure() {
      const card = track.querySelector(".azc-review-card");
      if (!card) return;
  ...
  if (window.__azcPageInit) {
    init();
  } else {
    window.__azcPageInit = true;
    if (document.readyState === "loading") {
