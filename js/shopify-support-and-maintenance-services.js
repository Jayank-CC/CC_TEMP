(function () {
  ...
  function initAccordion() {
    const items = document.querySelectorAll(".ssm-acc-item");
    if (!items.length) return;

    items.forEach(function (item) {
      const btn = item.querySelector(".ssm-acc-btn");
      const panel = item.querySelector(".ssm-acc-panel");
      if (!btn || !panel || btn.dataset.ssmBound === "1") return;
      btn.dataset.ssmBound = "1";
      ...
      btn.addEventListener("click", function () {
        ...
        items.forEach(function (other) {
          ...
          const otherBtn = other.querySelector(".ssm-acc-btn");
          const otherPanel = other.querySelector(".ssm-acc-panel");
  ...
  function initConsultForm() {
    const form = document.querySelector(".ssm-consult-form");
    if (!form || form.dataset.ssmBound === "1") return;
    form.dataset.ssmBound = "1";

    form.addEventListener("submit", function (e) {
      e.preventDefault();
      const honeypot = form.querySelector('input[name="ssm_hp"]');
      if (honeypot && honeypot.value) return;

      const success = form.querySelector(".ssm-form-success");
  ...
  function reviewCarousel() {
    const viewport = document.querySelector(".ssm-review-viewport");
    const track = document.querySelector(".ssm-review-track");
    ...
    function measure() {
      const card = track.querySelector(".ssm-review-card");
      if (!card) return;
  ...
  if (window.__shopifySupportMaintenancePageInit) {
    init();
  } else {
    window.__shopifySupportMaintenancePageInit = true;
    if (document.readyState === "loading") {
