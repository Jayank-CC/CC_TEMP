(function () {
  ...
  // ---- consultation form (fake success, honeypot check) ----
  function initConsultForm(root) {
    const form = root.querySelector(".m365-consult-form");
    if (!form || form.dataset.m365Bound === "1") return;
    form.dataset.m365Bound = "1";

    form.addEventListener("submit", function (e) {
      e.preventDefault();
      const honeypot = form.querySelector('input[name="m365_hp"]');
      if (honeypot && honeypot.value) return;

      const success = form.querySelector(".m365-form-success");
  ...
  function reviewCarousel() {
    const viewport = document.querySelector(".m365-review-viewport");
    const track = document.querySelector(".m365-review-track");
    ...
    function measure() {
      const card = track.querySelector(".m365-review-card");
      if (!card) return;
  ...
  function init() {
    const page = document.querySelector(".m365-page");
    if (page) initConsultForm(page);
    reviewCarousel();
  }

  if (window.__m365PageInit) {
    init();
  } else {
    window.__m365PageInit = true;
    if (document.readyState === "loading") {
