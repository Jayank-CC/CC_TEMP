(function () {
  ...
  // shared review-card content pool), renamed from the `ecom-` prefix to `mkt-` to match this
  // page's own class names.
  const viewport = document.querySelector(".mkt-review-viewport");
  const track = document.querySelector(".mkt-review-track");
  ...
  function measure() {
    const card = track.querySelector(".mkt-review-card");
    if (!card) return;
  ...
  if (window.__mktPageInit) {
    build();
  } else {
    window.__mktPageInit = true;
