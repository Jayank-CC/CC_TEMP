(function () {
  ...
  function initAccordion() {
    const items = document.querySelectorAll(".shpm-acc-item");
    if (!items.length) return;

    items.forEach(function (item) {
      const btn = item.querySelector(".shpm-acc-btn");
      const panel = item.querySelector(".shpm-acc-panel");
      if (!btn || !panel || btn.dataset.shpmBound === "1") return;
      btn.dataset.shpmBound = "1";
      ...
      btn.addEventListener("click", function () {
        ...
        items.forEach(function (other) {
          ...
          const otherBtn = other.querySelector(".shpm-acc-btn");
          const otherPanel = other.querySelector(".shpm-acc-panel");
  ...
  function initConsultForm() {
    const form = document.querySelector(".shpm-consult-form");
    if (!form || form.dataset.shpmBound === "1") return;
    form.dataset.shpmBound = "1";

    form.addEventListener("submit", function (e) {
      e.preventDefault();
      const honeypot = form.querySelector('input[name="shpm_hp"]');
      if (honeypot && honeypot.value) return;

      const success = form.querySelector(".shpm-form-success");
  ...
  function reviewCarousel() {
    const viewport = document.querySelector(".shpm-review-viewport");
    const track = document.querySelector(".shpm-review-track");
    ...
    function measure() {
      const card = track.querySelector(".shpm-review-card");
      if (!card) return;
  ...
  if (window.__shopifyMigrationPageInit) {
    init();
  } else {
    window.__shopifyMigrationPageInit = true;
    if (document.readyState === "loading") {
